...
namespace MISA.HUST._21H._2022.API.Controllers
{
    ...
    public class EmployeesController : ControllerBase
    {
        ...
        private const string CreatedBy = "Nguyễn Văn Mạnh";
        private const string ModifiedBy = "Nguyễn Văn Mạnh";
        ...
        public IActionResult GetAllEmployees()
        {
            try
            {
                ...
                // Tạo truy vấn SQL
                string query = "SELECT * FROM employee e ORDER BY e.ModifiedDate DESC";
        ...
        /// </returns>
        /// Note: pageNumber = 0 thì lấy dữ liêu cho trang cuối
        [HttpGet]
        [Route("filter")]
        ...
        public IActionResult UpdateEmployee([FromBody] Employee employee, [FromRoute] Guid employeeId)
        {
            try
            {
                ...
                // Kiểm tra format mã nhân viên
                //if (!(employee.EmployeeCode.Length > 2 && employee.EmployeeCode.StartsWith("NV")))
                //{
                //    ErrorInfo errorInfo = new(errorCode: "e002", errorMessage: "Mã nhân viên phải có dạng NV<số tự nhiên>");
                ...
                //   "Gender = @Gender, IdentityNumber = @IdentityNumber, IdentityDate = @IdentityDate, " +
                //   "IdentityPlace = @IdentityPlace, " +
                //   "Email = @Email, PhoneNumber = @PhoneNumber, " +
                //   "PositionName = @PositionName, " +
                //   "PositionId = (SELECT PositionId FROM positions WHERE PositionName = @PositionName), " +
                //   "DepartmentName = @DepartmentName, " +
                //   "DepartmentId = (SELECT DepartmentId FROM department WHERE DepartmentName = @DepartmentName), " +
                //   "PersonalTaxCode = @PersonalTaxCode, Salary = @Salary, JoinDate = @JoinDate, " +
                //   "WorkStatus = @WorkStatus, ModifiedDate = @ModifiedDate, ModifiedBy = @ModifiedBy "
                //   + "WHERE EmployeeId = @EmployeeId;";
